"""
Setup and initial ingestion script.
Run this once to initialize the system and build the vector store.
...
def check_environment():
    ...
    if not docs:
        logger.warning(f"⚠️  No documents found in {KNOWLEDGE_DIR}")
...
def main():
    ...
    if not check_environment():
        logger.warning("⚠️  Environment check found issues, but continuing...")
    ...
    if args.full:
        ...
        if not INDEX_DIR.exists():
            logger.warning("⚠️  Vector store not found!")
